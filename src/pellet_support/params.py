...
class SupportGenParams:
    ...
    #: 모델 아랫면과 서포터 사이에 둘 '실제' 틈(mm).
    #:
    #: 예전에는 층 개수로만 지정했는데, 층높이가 구슬 크기에 비례해서
    #: 굵은 펠릿에서는 1층이 1.3mm 나 되는 큰 틈이 됐다. 여기에 구슬 반지름
    #: 만큼의 클리핑까지 겹쳐 실측 4.76mm 가 비었다(설계 의도는 1.3mm).
    #: mm 로 지정하면 구슬 크기와 무관하게 일정한 틈을 유지한다.
    contact_z_gap_mm: float = 0.3
    contact_z_gap_layers: int = 0
    ...
    #: 두 가지를 하나로 합칠 최대 수평 거리. None 이면 구슬 지름의 6배.
    ...
    #: 접점 바로 아래 트렁크의 최소 굵기(몸통 구슬 지름의 배수). 세장비
    #: 조건은 접점에서 0으로 수렴하므로, 세장비를 아무리 낮춰도 접점
    #: 바로 아래 마지막 구간은 항상 구슬 하나짜리 외줄로 남는다. 1.0보다
    #: 크게 주면 그 구간에도 최소 굵기를 강제해 외줄을 없앤다. 밑동
    #: 굵기(세장비가 정하는 값)에는 영향이 없다 — 밑동은 이미 이보다 굵다.
    tree_tip_diameter_ratio: float = 1.0
    #: 기본 구슬로는 자리(수직 틈)가 안 나오는 접점을 다시 시도할 더 작은
    #: 구슬 지름(mm). None 이면 끔(그런 접점은 그냥 빠진다). 한 가지(트렁크)
    #: 안에서는 여전히 구슬 크기가 하나로 균일하고, 이 보완 구슬만 쓰는
    #: 트렁크가 별도로 자란다 — 서로 다른 가지가 서로 다른 굵기를 쓰는 것.
    tree_fallback_bead_diameter_mm: Optional[float] = None
    ...
    #: 보통 구슬 지름을 쓴다(파이프라인이 자동으로 채운다).
    removal_opening_mm: float = 1.0
    ...
    #: 남은 틈을 더 작은 구슬로 몇 세대까지 메울지.
    fill_generations: int = 3
    #: 한 세대 내려갈 때 지름을 곱할 비율.
    fill_shrink: float = 0.7
    #: 구슬로 끝내 못 채운 자리를 일반 서포터(얇은 벽)로 메울지.
    fallback_solid: bool = True
    #: 그 벽을 세울 최소 면적(mm^2). 너무 작은 조각은 무시한다.
    fallback_min_area_mm2: float = 0.5
    #: 인쇄 가능한 최소 구슬 지름(mm). None 이면 노즐 * 아래 비율.
    min_bead_diameter_mm: Optional[float] = None
    min_bead_to_nozzle_ratio: float = 0.35
    #: --auto(격자 모드) 가 구슬 지름 상한을 정하는 기준. 예전에는 '노즐
    #: 지름의 절반'으로 무조건 눌러서, 노즐과 무관하게 원래 넓은 빈 공간
    #: (오버행 단면이 큰 모델)에서도 구슬을 키우지 못해 개수가 불필요하게
    #: 늘었다. 대신 매 층 전체를 채워야 하는 오버행 단면의 평균 지름에 이
    #: 비율을 곱한 값을 상한으로 쓴다. 최종 선택은 여전히
    #: fillable_fraction(품질) 기준으로 하므로, 상한을 넉넉히 열어도 좁은
    #: 곳까지 큰 구슬이 강제되지 않는다 — 상한은 탐색 범위를 넓혀줄 뿐이다.
    auto_void_bead_ratio: float = 0.35
    #: --auto(트리 모드) 가 구슬 지름 상한을 정하는 기준.
    #:
    #: 트리는 격자와 달리 접점에서 베드까지 내려가는 '기둥'을 세운다.
    #: 기둥이 채워야 할 빈 공간의 크기는 오버행 단면의 폭이 아니라 베드
    #: 까지의 높이다 — 배 선체처럼 단면은 얇고 긴 고리 모양이어도 높이가
    #: 크면 구슬을 그만큼 많이 쌓아야 한다. 단면 폭 기준(auto_void_bead_
    #: ratio)을 그대로 쓰면 높이가 큰 모델에서도 상한이 작게 잡혀 구슬이
    #: 계속 작게 나온다. 그래서 오버행의 (면적 가중) 평균 높이에 이 비율을
    #: 곱한 값을 상한으로 쓴다. 높이가 지름보다 보통 훨씬 크므로, 같은
    #: 크기의 구슬을 얻으려면 auto_void_bead_ratio 보다 작은 값을 쓴다.
    auto_tree_height_ratio: float = 0.1
    #: 모델을 전혀 받치지 않는 외톨이 구슬 뭉치를 지울지.
    #: 구슬이 최소 몇 개의 이웃과 닿아야 남길지. 0 이면 정리 안 함.
    #: 이웃 1~2개짜리는 힘을 전달하지 못하고 노즐에 걸려 떨어져 나간다.
    min_bead_contacts: int = 3
