...
def grow_branches(
    ...
    **모든 이동은 기울기 한계 안에서만 일어난다.** 이것이 예전 구현과의
    ...
    def land(tip):
        """더 못 내려가는 가지를 모델 윗면 바로 위에 내려앉힌다."""
        zz = tip["z"]
        i = field.index(zz)
        while i - 1 >= 0 and heights[i - 1] > z_end and \
                not field.blocked(tip["x"], tip["y"], heights[i - 1]):
            i -= 1
        low = max(z_end, heights[i])
        if low < tip["z"] - 0.25 * r_bead:
            node = skeleton.add_node(tip["x"], tip["y"], low, None, bed_radius,
                                     field.index(low), kind="trunk")
            skeleton.reparent(tip["node"], node)
            tip["node"] = node
        skeleton.nodes[tip["node"]].on_model = True
    ...
    for _ in range(max_iter):
        ...
        if tree is not None:
            reach = 2.0 * max(leans) if leans else 0.0
            pairs = sorted(
