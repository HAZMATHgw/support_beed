        ...
        if not on_bed:
            ...
            if mesh is not None:
                try:
                    ...
                    # 예전에는 intersects_any 로 '아래 어딘가에 모델이 있는가'만
                    # 봤다. 그러면 선체 한참 위 허공에 뜬 구슬도 저 아래 선체가
                    # 걸려서 '모델에 얹혔다'로 통과했다. 실제로 받쳐지려면
                    # 바로 아래에 있어야 하므로 첫 충돌까지의 거리를 본다.
                    locs, idx_ray, _ = mesh.ray.intersects_location(
                        origins, dirs, multiple_hits=False
                    )
                    if len(idx_ray):
                        drop = origins[idx_ray][:, 2] - locs[:, 2]
                        max_drop = (
                            (gen.contact_z_gap_layers + 1) * gen.layer_height_mm
                            + 0.5 * contact.bead_diameter_mm
                        )
                        close = drop <= max_drop
                        on_model[idx_ray[close]] = True
                except Exception:
                    pass
