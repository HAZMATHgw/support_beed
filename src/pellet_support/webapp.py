...
#: 요청 하나가 이 시간(초)을 넘기면 계산을 기다리지 않고 먼저
#: 응답한다. 3MF 파싱 자체가 큰 파일에서 20초 넘게 걸릴 수 있어서,
#: 우리 코드가 아무리 빨라도 이 시간보다 짧게는 못 끝낼 때가 있다.
REQUEST_TIME_BUDGET_S = 45
...
def api_generate():
    ...
    try:
        ...
        gen = SupportGenParams(
            ...
            fallback_solid=not _form_bool("no_fallback_solid"),
            max_layers=_form_int("max_layers", 4000),
        )

        @copy_current_request_context
        def do_heavy_work():
            """무거운 작업 전체(파일 로딩~서포터 생성)를 한데 묶는다.

            타임아웃으로 감싸기 위해 별도 함수로 뺐다. 3MF 는 XML 파싱
            자체가 느려서(실측: 삼각형 192만 개 파일에서 20.5초, 그중
            13초가 순수 파싱), 이건 우리 계산이 시작되기도 전에 이미
            걸리는 시간이다. quick_overhang_estimate 만으로는 못 잡는다.
            """
            mesh = load_mesh(in_path)
            mesh.apply_translation([0, 0, -mesh.bounds[0][2]])

            local_gen, local_contact, local_body = gen, contact, body
            local_tuning = None
            if (_form_bool("auto_tune")
                    and _form_optional_float("bead_diameter") is None):
                local_tuning = auto_tune_bead_diameter(
                    mesh, local_gen, local_contact,
                    target_fill=_form_float("auto_target", 0.75),
                )
                local_contact, local_body = make_params(
                    nozzle_diameter_mm=_form_float("nozzle", 1.0),
                    bead_diameter_mm=local_tuning.chosen.bead_diameter_mm,
                    overlap=_form_float("overlap", 0.08),
                    lateral_overlap=_form_optional_float("lateral_overlap"),
                    vertical_overlap=_form_optional_float("vertical_overlap"),
                    body_bead_ratio=_form_float("body_bead_ratio", 0.97),
                ...
                local_gen = dataclasses.replace(
                    local_gen, layer_height_mm=local_contact.layer_height_mm())

            with warnings.catch_warnings(record=True) as caught:
                warnings.simplefilter("always")
                local_result = generate_support(
                    mesh, local_gen, local_contact, local_body,
                    detail=_form_int("sphere_detail", 1),
                    verbose=False,
                )
            return mesh, local_gen, local_contact, local_result, local_tuning, caught

        # 요청 전체에 상한 시간을 건다. 어느 단계(파일 파싱이든 계산이든)가
        # 느리든 이 시간 안에는 반드시 응답한다 — 브라우저·프록시가 먼저
        # 연결을 끊어서 "서버가 죽었다"로 보이는 것보다, 우리가 먼저 사정을
        # 설명하는 게 낫다.
        #
        # ThreadPoolExecutor 의 with 블록을 쓰면 __exit__ 이 기본적으로
        # shutdown(wait=True) 를 호출해서, 시간 초과를 잡아내도 스레드가
        # 실제로 끝날 때까지 응답이 미뤄진다(실측: 타임아웃 5초로 걸어도
        # 응답이 22초 뒤에야 나갔다). 데몬 스레드 + 큐로 바꿔서, 진짜로
        # 제한 시간이 지나면 스레드를 기다리지 않고 바로 응답한다.
        result_queue: "queue.Queue" = queue.Queue(maxsize=1)

        def runner():
            try:
                result_queue.put(("ok", do_heavy_work()))
            except Exception as exc:  # noqa: BLE001 - 스레드 안 예외를 큐로 전달
                result_queue.put(("error", exc))

        threading.Thread(target=runner, daemon=True).start()
        try:
            status, payload = result_queue.get(timeout=REQUEST_TIME_BUDGET_S)
        except queue.Empty:
            return jsonify(
                error=(
                    f"{REQUEST_TIME_BUDGET_S}초가 지나도 끝나지 않았습니다. "
                    f"파일이 크거나(특히 3MF 는 STL보다 읽는 데 몇 배 오래 "
                    f"걸립니다) 구슬 수가 많은 것으로 보입니다.\n"
                    f"해결 방법: 가능하면 STL로 변환해서 다시 올리거나, "
                    f"--bead-diameter 를 키우거나, 터미널에서 pellet-support "
                    f"명령을 직접 실행하세요(터미널은 이 시간 제한이 없습니다)."
                )
            ), 504
        if status == "error":
            raise payload
        mesh, gen, contact, result, tuning, caught = payload
        ...
        if not gen.tree_enabled:
            # 격자 지표(균일한 pitch, 배위수 12)는 트리 골격에는 적용되지
            # 않는다. 트리 구슬은 가지 경로를 따라 굵기와 간격이 제각각이라
            # 그대로 재면 배위수 0 같은 오해하기 쉬운 값이 나온다.
            try:
