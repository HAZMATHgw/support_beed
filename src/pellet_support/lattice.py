...
def support_bead_generate_centers(
    ...
    edge_margin_override: Optional[float] = None,
    ...
    # 가장자리 여백은 '구슬이 모델에 부딪히지 않을 만큼'만 있으면 된다.
    # 서포터 영역은 이미 모델에서 xy_clearance 만큼 떨어져 있으므로,
    # 실제로 더 깎아야 하는 양은 (구슬 반지름 - xy_clearance) 뿐이다.
    # 지름의 고정 비율로 깎으면 필요 이상으로 영역이 줄어 구슬이 통째로
    # 안 들어가고, 그 층에서 사슬이 끊긴다.
    edge_margin = (edge_margin_override if edge_margin_override is not None
                   else params.bead_diameter_mm * params.edge_margin_ratio)
    ...
    for part in parts:
        ...
        if not shrunk.is_empty:
            safe_parts.append(shrunk)
        elif edge_margin <= 0:
            # 여백이 필요 없는 경우에만 원본을 그대로 쓴다.
            safe_parts.append(part)
        # 여백이 필요한데 조각이 그보다 얇으면 이 구슬로는 못 채운다.
        # 원본을 그대로 쓰면 구슬이 모델을 파고든다(실측 4.9% 충돌).
