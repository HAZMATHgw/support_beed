...
def generate_support(
    ...
    for j in range(n_bead):
        ...
        # 걸쳐서, 실측 25~59개가 충돌했다).
        # 구슬이 걸치는 z 범위의 모델을 모두 빼서 그런 충돌을 원천 차단한다.
        # 구슬 위쪽으로는 '떼어낼 틈'(contact_z_gap_mm)까지 더 비워 둔다.
        # 아래쪽은 구슬이 차지하는 만큼만 보면 된다.
        k_lo = min(len(det_slices) - 1, max(0, int((z - r0 - z0) / det_h)))
        k_hi = min(len(det_slices) - 1,
                   max(0, int((z + r0 + gen.contact_z_gap_mm - z0) / det_h)))
        if k_hi > k_lo and not clean(region).is_empty:
            ...
            if spanned:
                ...
                region = clean(region).difference(blocked)

        bead_support.append(region)
