...
각 노드가 떠받치는 접촉점 수의 **제곱근**에 비례해 굵기를 정합니다(단면적이
부하에 비례해야 하므로). 선형으로 키우면 접촉점 300개짜리 트렁크가 말단의
300배가 되어버립니다. 실측: 말단 2.42mm, 트렁크 6.06mm.

### 구현상 알려진 한계

- **충돌 회피는 16방향 우회 탐색입니다.** 모델에 막히면 뚫린 방향 중 가장
  가까운 곳으로 돌아 내려갑니다. 완전한 경로 탐색(A* 등)은 아니지만
  "한 층 내려갈 자리"만 찾으면 되므로 실용적으로 충분합니다.
  실측: 배 모델에서 접촉점 52개 → 트렁크 5개, **갇힌 가지 0개**
  (우회 도입 전에는 트렁크 7개 중 3개가 z=9.60/1.05/1.05 에서 갇혔습니다).

  이때 우회 반경을 기울임 한계에 묶으면 안 됩니다 — 기울임은 "한 층에
  얼마나 비스듬히 갈까"이고 우회는 "장애물을 벗어나려면 얼마나 옆으로
  가야 하나"라 크기가 전혀 다릅니다. 실측에서 0.58mm 만 훑고 포기해
  선체 벽을 못 넘던 문제가 있었습니다.
